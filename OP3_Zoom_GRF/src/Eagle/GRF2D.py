"""
GRF2D provides the following functions:
- udpate the field.
- assimilate data.
...
class GRF:
    ...
    def __init__(self) -> None:
        # s0: construct grf grid.
        self.__construct_grf_grid()

        # s1: compute matern kernel.
    ...
    def __construct_grf_grid(self) -> None:
        """
        Construct the grf grid using hexgonal discretiation.
        """
        self.__grf_grid = np.empty([0, 3])

        # s0: load polygon and depth layers.
        plg_border = self.__field.get_polygon_border()
        depth_layers = self.__field.get_depth_layers()

        # s1: get the border limit for the polygon.
        xmin, ymin = map(np.amin, [plg_border[:, 0], plg_border[:, 1]])
        xmax, ymax = map(np.amax, [plg_border[:, 0], plg_border[:, 1]])
        ygap = self.__spatial_resolution * cos(radians(60)) * 2
        xgap = self.__spatial_resolution * sin(radians(60))
        no_depth_layers = len(depth_layers)

        # s2: build regular grid first and then shift the every second row to construct hexgonal grid.
        ...
        for i in range(len(gy)):
            for j in range(len(gx)):
                if j % 2 == 0:
                    ...
                    y = gy[i]
                p = np.array([x, y])
                if self.__field.is_location_within_border(p):
                    grid2d.append([x, y])
                    counter_grid2d += 1
        for i in range(no_depth_layers):
            for j in range(counter_grid2d):
                self.__grf_grid = np.append(self.__grf_grid, np.array([grid2d[j][0], grid2d[j][1],
                                                                       depth_layers[i]]).reshape(1, -1), axis=0)
        self.__grf_grid = np.array(self.__grf_grid)

        # s3: prepare for the later calcuation.
        self.__Ngrid = len(self.__grf_grid)
        self.__Fgrf = np.ones([1, self.__Ngrid])
        self.__xg = vectorize(self.__grf_grid[:, 0])
        self.__yg = vectorize(self.__grf_grid[:, 1])

    def __construct_grf_kernel(self):
        """ Construct distance matrix and thus Covariance matrix for the kernel. """
        self.__distance_matrix = cdist(self.__grf_grid, self.__grf_grid)
